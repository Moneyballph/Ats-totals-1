# Moneyball Phil — ATS & Totals App (Final Stable v3.3 — Auto Volatility + Fixed Spread)
...
with col_inputs:
    ...
    with st.form("inputs_form", clear_on_submit=False):
        ...
        with st.expander("⚙️ Advanced adjustments (optional)", expanded=False):
            ...
            with u3:
                # Auto volatility toggle + manual number; resolved to effective value when running
                ...
                variance_pct_manual = st.number_input(
                    "Volatility tweak (±% SD)", value=0.0, step=5.0, format="%.0f",
...
with col_results:
    ...
    if run_projection:
        ...
        # Resolve effective volatility (exactly as before)
        auto_vol_used = suggested_volatility(sport) if auto_volatility else float(variance_pct_manual)

        # Apply adjustments (all controls wired exactly as before)
        ...
        # -------- FIXED SPREAD CALCS (bug fix only) --------
        if sd_margin > 0:
            home_line = float(S.spread_line_home)
            away_line = -home_line

            # Home covers if margin >= -home_line (e.g., -2.5 -> margin >= +2.5)
            home_cover_threshold = -home_line
            z_home = (home_cover_threshold - proj_margin) / sd_margin
            true_home = (1.0 - _std_norm_cdf(z_home)) * 100.0

            # Away covers if margin <= away_line
            z_away = (away_line - proj_margin) / sd_margin
            true_away = _std_norm_cdf(z_away) * 100.0
        else:
            true_home = true_away = 50.0
        # ---------------------------------------------------
