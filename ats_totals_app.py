# Moneyball Phil — ATS & Totals App (Final v3.3 — Fixed Spread Probabilities + Full Reset)
...
if st.session_state.get("last_sport") != sport:
    for key in st.session_state.keys():
        if isinstance(st.session_state[key], (int, float)): st.session_state[key] = 0.0
        elif isinstance(st.session_state[key], str): st.session_state[key] = ""
    ...
    with st.form("inputs_form",clear_on_submit=False):
        ...
        with st.expander("⚙️ Advanced adjustments (optional)", expanded=False):
            ...
            if sport in ["NFL","NCAA Football"]:
                ...
                with f2: redzone_H_pct=st.number_input("Home red zone (±% PF)",value=0.0,step=1.0,format="%.0f")
                with f3: redzone_A_pct=st.number_input("Away red zone (±% PF)",value=0.0,step=1.0,format="%.0f")
            else: plays_pct=redzone_H_pct=redzone_A_pct=to_margin_pts=0.0
            if sport in ["NBA","NCAA Basketball"]:
            ...
            else: pace_pct_hoops=ortg_H_pct=ortg_A_pct=drtg_H_pct=drtg_A_pct=rest_H_pct=rest_A_pct=0.0
            if sport=="MLB":
            ...
            else: sp_H_runs=sp_A_runs=bullpen_H_runs=bullpen_A_runs=park_total_pct=weather_total_pct=0.0
        # Form actions
        c1,c2=st.columns(2)
        with c1: reset_clicked=st.form_submit_button("Reset Inputs")
        with c2: run_projection=st.form_submit_button("🔮 Run Projection")
...
with col_results:
    ...
    if run_projection:
        ...
        proj_total=home_pts+away_pts; proj_margin=home_pts-away_pts
        rows=[]
        # Spread (home)
        ...
        st.session_state.proj_total=proj_total; st.session_state.proj_margin=proj_margin
        st.session_state.proj_home_pts=home_pts; st.session_state.proj_away_pts=away_pts
        st.session_state.auto_vol_used=auto_vol_used; st.session_state.auto_vol_mode=bool(auto_volatility)
    if st.session_state.results_df is not None:
        df=st.session_state.results_df
        proj_total=st.session_state.proj_total; proj_margin=st.session_state.proj_margin
        home_pts=st.session_state.proj_home_pts; away_pts=st.session_state.proj_away_pts
        auto_vol_used=st.session_state.get("auto_vol_used",None); auto_vol_mode=st.session_state.get("auto_vol_mode",False)
        S=st.session_state
        ...
        if auto_vol_used is not None:
            ...
            st.caption(f"Volatility applied: **{mode_txt} {auto_vol_used:.0f}%**")
        ...
        if save_straight:
            st.success("✅ Bet saved (copy row below).")
            st.code(f"{datetime.date.today()}, Straight, {selected['Bet Type']}, {int(selected['Odds'])}, {selected['True %']:.1f}%, {selected['Implied %']:.1f}%, {S.stake:.2f}, W/L, +/-$, {selected['EV %']:.1f}%, Cumulative, ROI%")
        if add_parlay:
            st.session_state.parlay_slip.append([selected["Bet Type"],int(selected["Odds"]),float(selected["True %"]),float(selected["Implied %"]),float(selected["EV %"]),tier_name])
...
if len(st.session_state.parlay_slip)==0:
    ...
    def american_to_decimal(odds: float)->float: return 1+(odds/100) if odds>0 else 1+(100/abs(odds))
    def american_to_implied_pct(odds: float)->float: return (100/(odds+100))*100 if odds>0 else (abs(odds)/(abs(odds)+100))*100
    dec_product=1.0; true_prod=1.0; implied_prod=1.0
    for leg in st.session_state.parlay_slip:
        leg_odds=int(leg[1]); dec_product*=american_to_decimal(leg_odds); true_prod*=float(leg[2])/100.0; implied_prod*=float(leg[3])/100.0
    combined_true_pct=true_prod*100.0; combined_implied_pct=implied_prod*100.0
    ...
    with colA: manual_parlay_odds=st.number_input("Book Parlay Odds (American)",value=0,step=1)
    with colB:
        if manual_parlay_odds!=0: st.metric("Book Parlay Odds (Decimal)",f"{american_to_decimal(manual_parlay_odds):.2f}")
        else: st.write(" ")
    if manual_parlay_odds!=0:
        book_implied_pct=american_to_implied_pct(manual_parlay_odds); ev_vs_book=combined_true_pct-book_implied_pct
        with colC: st.metric("Book Implied Probability",f"{book_implied_pct:.1f}%")
        simple_bar("EV% vs Book",ev_vs_book); st.caption("EV% vs Book = True Parlay % − Book Implied %")
    else: st.caption("Enter Book Parlay Odds above to compute EV vs. book price.")
    st.markdown("### Manage Parlay")
    rm_leg=st.selectbox("Remove a leg (optional):",options=["—"]+[r[0] for r in st.session_state.parlay_slip])
    if rm_leg!="—":
        st.session_state.parlay_slip=[leg for leg in st.session_state.parlay_slip if leg[0]!=rm_leg]; st.success(f"Removed: {rm_leg}")
